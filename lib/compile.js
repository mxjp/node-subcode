...
async function compileCode(src, options = {}) {
	...
	let templateBody = '';
	const context = vm.createContext(options.compileContext ? Object.create(options.compileContext) : {});
	context.include = (name, request) => {
		...
		return compileCodeFromFile(request, options).then(code => {
